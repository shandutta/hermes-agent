...
def configure_account(account: str | None):
    """Route API calls to a named Google account token when requested."""
    global TOKEN_PATH
    if not account:
        return
    TOKEN_PATH = HERMES_HOME / "google_tokens" / f"{_account_slug(account)}.json"
...
def _extract_message_body(msg: dict) -> str:
    """Extract a useful body from Gmail's nested MIME tree.

    Marketing/newsletter emails often put text/html or text/plain several
    levels deep under multipart/alternative inside multipart/related. The old
    one-level scan returned an empty body for those messages, which made
    newsletter automations look like sources had gone dark.
    """
